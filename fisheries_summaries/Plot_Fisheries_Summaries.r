...
setwd("//penguin/assessments/skj/2016/assessment/Data_preperation/Fisheries_Structure")
...
# List below explicitely links the 33 fisheries to the regions they are in. Note some in R7 don't cover the whole region so have to state thier subregions
    ...
    regHigh <- lapply(1:23, function(x) reg_defs$MufArea[fsh.reg[x] == reg_defs$reg])
...
# Structure all the data
    ...
    Cdat = ExpandDat(dat=Cdat, year.lab="yr", fisheries=fisheries, first.yr=1975, last.yr=2015)    # calls function that cleans and reshapes the data

    Ldat = read.csv(lfile, header=TRUE)                                         # Loads length data
    Ldat = ExpandDat(dat=Ldat, year.lab="yr", fisheries=fisheries, first.yr=1975, last.yr=2015)   # calls function that cleans and reshapes the data
    ...
    L.avail = ifelse(is.na(match(fisheries, Ldat$fsh))=='TRUE',0,1)     # creates variable which indicates the availability of data, prevents crasing if there's no data
